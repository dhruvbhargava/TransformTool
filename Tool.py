...
class Window(widget):
    ...
    def Home(self):
        ...
        Load_btn.move(self.GridCell['horizontal']
                      * 1, self.GridCell['verticle']*1)
        InputLabel.move(self.GridCell['horizontal']
                        * 1, self.GridCell['verticle']*15)
        self.InputField.resize(
            self.GridCell['horizontal']*18, self.GridCell['verticle']*1.5)
        self.InputField.move(
            self.GridCell['horizontal']*1, self.GridCell['verticle']*16)
        Button.move(self.GridCell['horizontal']*1,
                    self.GridCell['verticle']*18)
        Reset_button.move(
            self.GridCell['horizontal']*15.5, self.GridCell['verticle']*18)
        # actions
    ...
    def TransformToolAction(self):
        updatedim = utils.transform_Image(
            self.Image_cv2, self.InputField.text())
